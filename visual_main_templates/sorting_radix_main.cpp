...
int main(int argc, char **argv)
{
    ...
    auto values = parse_int_list(json_string_value(json, "values", "170 45 75 90 802 24 2 66"));

    DSA_VIS_BEGIN(trace_path);
    DSA_VIS_CTX("sorting_radix");
    DSA::Sorting::Vis::PrimaryScope<std::vector<int>::iterator> scope(values.begin(), values.end(), "A");
    DSA::Sorting::IntRadixSort(values.begin(), values.end());
    DSA_VIS_END();
    return 0;
}
